...
Technically, you could use "classless IN-ADDR.ARPA delegation" (see below), but this is not recommended for a full class C because the customer will (rightly) expect a standard reverse delegation (as above).
...
Your customer would set up a zone called "64/30.33.22.11.in-addr.arpa" on his DNS server with the following records:
...
Technically, you could create another level of CNAME redirection similar to "classless IN-ADDR.ARPA delegation" (see above), but this is not recommended because the redirection becomes too deep.
