...
With the configuration in the screenshot above, out of 10 requests, 1 response would point to 1.1.1.1, 3 responses to 3.3.3.3 and 6 responses to 6.6.6.6.

This plug-in works with both IPv4 and IPv6 addresses.

NOTE: DNS round-robin cannot provide an exact weight distribution of traffic because other DNS servers will cache the records and potentially serve multiple end users with the same data, and different end users may use services more or less. But measured on average over time, the traffic distribution will be as weighted.
