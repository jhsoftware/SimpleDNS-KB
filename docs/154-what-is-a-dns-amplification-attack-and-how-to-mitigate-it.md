...
In a DNS amplification attack, a large number of DNS requests are sent with a spoofed from-IP-address to one or more DNS servers.  
...
The victim of the attack is not able to tell where the attack originates from - to him, it appears that the attackers are the DNS servers.  
...
a) Lame requests (recursive request for non-local domain names from an IP address that the DNS servers do not recurse for)  
b) Requests for &lt;root&gt;  
