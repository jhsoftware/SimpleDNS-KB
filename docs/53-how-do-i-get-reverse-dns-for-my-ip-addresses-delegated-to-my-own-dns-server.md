...
The owner can choose to sub-delegate reverse DNS for a range of IP addresses to someone else, who in turn can sub-delegate parts of that range further, etc.
...
And these registries delegate their IP addresses to backbone providers and ISPs, who delegate to end-users.
...
If you have less than a class C (256 IP addresses), then the reverse DNS sub-delegation uses a slightly different format called "Classless IN-ADDR.ARPA delegation". The exact zone name format used for this varies for each ISP, so it is very important that you set up your reverse zone on your DNS server with the zone name provided by the ISP.  
