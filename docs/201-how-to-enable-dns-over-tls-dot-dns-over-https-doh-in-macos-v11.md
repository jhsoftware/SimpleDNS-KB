...
Configuring this in MacOS (v. 11 / BigSur or later) requires installing a "configuration profile" file (a file with a ".mobileconfig" extension), containing data about the DNS server(s) to use.
...
Enter your company name, select the protocol (DoT or DoH), enter your DNS server host name or query URL, and the DNS server IP addresses, click the Download button, and click/open the downloaded file:
