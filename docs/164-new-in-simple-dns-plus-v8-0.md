...
Note: The DNSSEC key information provided through the HTTP API does NOT include the private cryptographic key data of the DNSSEC keys - only metadata about them.
...
- **Moved function to generate DNSSEC DS-records (for inclusion in parent zone)**
    ...
    This function, which used to be part of the "DNSSEC Sign Zone" dialog, has been moved to the DNSSEC dropdown button / menus.
...
- **Moved DNSSEC signature expiration time field**
    ...
    This setting, which used to be in the "DNSSEC Key" dialog, has been moved to the DNSSEC Sign Zone dialog and Zone Properties dialog / DNSSEC tab / automatic DNSSEC signing.
