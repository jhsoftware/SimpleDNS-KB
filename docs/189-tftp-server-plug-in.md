...
IMPORTANT: Make sure to specify a folder on a local harddisk as the service is not able to access network shares.
- **Max. connections**  
...
You may need to adjust this setting if your network limits UDP packet size.
- **Default timeout**  
How long the server should wait for acknowledgment that a client received the last data block.  
...
Note that in recent Windows versions this first needs to be enabled from the Windows Control Panel / Programs &amp; Features / Turn Windows features on or off / TFTP client.
