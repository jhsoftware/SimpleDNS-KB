...
The data of this TXT-record always starts with "v=DMARC1; ...", and may contain several fields describing what action to take, domain alignment, reporting, etc.

Required fields are "v" (value always "DMARC1") and "p". The value for "p" (policy) can be either "none", "quarantine", or "reject" - i.e. what the receiving e-mail server should do if a message does not pass SPF or DKIM tests.
