...
Simple DNS Plus v. 5.1 and later with the default configuration are not vulnerable to this, but earlier versions may be - see details below.
...
If your DNS server is only hosting domain names (authoritative server only), simply disable recursion, and you are safe.  
