...
If you are using a SIP server for VOIP communication and want to enable other Internet users to call people in your organization using addresses such as "1004@domain.com" (where 1004 is a local phone extension) or e-mail addresses such as "user@domain.com", you need to set up a DNS SRV-record for "_sip._udp." + your domain.

To set up SRV-records in Simple DNS Plus, in the main window click the "Records" button, then right-click a zone in the left list, select "Other new record...", and then "SRV-record".
